...
def create_transaction(
    ...
    # загружаем в переменную wallet_of_sender кошелек отправителя
    # если мы в тестовой сети, то загружаем кошелек из тестовой сети
    ...
    # создаем объект транзакции и сохраняем его тем самым в БД
    transaction = Transaction(sender=sender,
                              ...
                              tx_hash=tx_hash)
    return transaction  # возвращаем объект с транзакцией
...
def update_wallet_balance(wallet: pydantic_models.Wallet):
    ...
    testnet = wallet.private_key.startswith('c')
